...
{
    public partial class PaymentModeMST : BasePage
    {
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                PaymentModeData objpayemnt = new PaymentModeData();
                PaymentModeBO objpayementBO = new PaymentModeBO();
                objpayemnt.Code = txtcode.Text;
                objpayemnt.Descriptions = txtdescription.Text;
                ...
                if (ViewState["ID"] != null)
                {
                    ...
                    objpayemnt.PaymentModeID = Convert.ToInt32(ViewState["ID"].ToString());

                }
                int result = objpayementBO.UpdatePaymentModeDetails(objpayemnt);
                ...
                else if (result == 5)
                {
                    ...
                    clearall();
                    GvPaymentdetails.DataSource = GetPaymentModeDetails(1, 10);
        ...
        protected void GvPaymentdetails_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Edits")
                {
                    ...
                    objpayemnt.PaymentModeID = Convert.ToInt32(ID.Text);
                    objpayemnt.ActionType = EnumActionType.Select;
                    List<PaymentModeData> GetResult = objpayementBO.GetPaymentModeDetailsByID(objpayemnt);
                    if (GetResult.Count > 0)
                    {
                        txtcode.Text = GetResult[0].Code;
                        txtdescription.Text = GetResult[0].Descriptions;
                        ViewState["ID"] = GetResult[0].PaymentModeID;
                        btnsave.Text = "Update";
                        bindresponsive();
                    }
                }
                if (e.CommandName == "Deletes")
                {
                    ...
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = GvPaymentdetails.Rows[i];
                    Label PaymentModeID = (Label)gr.Cells[0].FindControl("lblID");
                    objpayemnt.PaymentModeID = Convert.ToInt16(PaymentModeID.Text);
                    objpayemnt.ActionType = EnumActionType.Delete;
                    int Result = objpayementBO.DeletePaymentModeDetailsByID(objpayemnt);
                    if (Result == 1)
                    {
        ...
        private void bindgrid(int index)
        {
            ...
            List<PaymentModeData> lstPay = GetPaymentModeDetails(index, pagesize);
            if (lstPay.Count > 0)
            {
        ...
        public List<PaymentModeData> GetPaymentModeDetails(int curIndex, int pagesize)
        {
            PaymentModeData objpayemnt = new PaymentModeData();
            PaymentModeBO objpayementBO = new PaymentModeBO();
            objpayemnt.Code = txtcode.Text == "" ? null : txtcode.Text;
            objpayemnt.Descriptions = txtdescription.Text == "" ? null : txtdescription.Text;
            objpayemnt.ActionType = EnumActionType.Select;
            objpayemnt.PageSize = pagesize;
            objpayemnt.CurrentIndex = curIndex;
            return objpayementBO.SearchPaymentModeDetails(objpayemnt);
        ...
        protected void ExportoExcel()
        {
            ...
            {
                wb.Worksheets.Add(dt, "Payment mode Type");
                Response.Clear();
                Response.Buffer = true;
                Response.Charset = "";
                Response.ContentType = "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet";
                Response.AddHeader("content-disposition", "attachment;filename= PaymentModeDetails.xlsx");
        ...
        protected DataTable GetDatafromDatabase()
        {
            ...
            List<PaymentModeData> PaymentTypeDetail = GetPaymentModeDetails(1, size);
            List<PaymentModeDatatoExcel> paymenttypetoexcel = new List<PaymentModeDatatoExcel>();
            int i = 0;
            foreach (PaymentModeData row in PaymentTypeDetail)
            {
                PaymentModeDatatoExcel EcxeclStd = new PaymentModeDatatoExcel();
                EcxeclStd.Code = PaymentTypeDetail[i].Code;
                EcxeclStd.Descriptions = PaymentTypeDetail[i].Descriptions;
