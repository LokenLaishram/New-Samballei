...
{
    public partial class VendorMST : BasePage
    {
        ...
        protected void BindDlls()
        {
            ...
            Commonfunction.PopulateDdl(ddl_vendotype, mstlookup.GetLookupsList(LookupNames.VendorType));
        }
        protected void ddl_vendotype_OnSelectedIndexChanged(object sender, EventArgs e)
        {
            bindgrid(1);
        }
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                VendorBO objBO = new VendorBO();
                obj.VendorTypeID = Convert.ToInt32(ddl_vendotype.SelectedValue == "" ? "0" : ddl_vendotype.SelectedValue);
                obj.VendorCode = txtVendorCode.Text == "" ? "null" : txtVendorCode.Text;
                obj.VendorName = txt_vendor.Text.Trim() == "" ? "" : txt_vendor.Text.Trim();
                obj.GSTIN = txtGSTIN.Text == "" ? "null" : txtGSTIN.Text;
                obj.ContactPerson = txtContactPerson.Text == "" ? "null" : txtContactPerson.Text;
                obj.PhoneNo = txtphoneNo.Text == "" ? "null" : txtphoneNo.Text;
                obj.Address = txtAddress.Text == "" ? "null" : txtAddress.Text;
                ...
                if (ViewState["ID"] != null)
                {
                    ...
                    obj.VendorID = Convert.ToInt32(ViewState["ID"].ToString());
                }
                int result = objBO.SaveVendor(obj);
        ...
        protected void Gv_vendor_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Edits")
                {
                    ...
                    //    return;
                    //}
                    VendorData objGroup = new VendorData();
                    VendorBO objVendorBO = new VendorBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gv_vendor.Rows[i];
                    Label ID = (Label)gr.Cells[0].FindControl("lblID");
                    objGroup.VendorID = Convert.ToInt32(ID.Text);
                    List<VendorData> GetResult = objVendorBO.GetVendorbyID(objGroup);
                    if (GetResult.Count > 0)
                    {
                        ddl_vendotype.SelectedValue = GetResult[0].VendorTypeID.ToString();
                        txt_vendor.Text = GetResult[0].VendorName;
                        txtVendorCode.Text = GetResult[0].VendorCode;
                        txtGSTIN.Text = GetResult[0].GSTIN;
                        txtphoneNo.Text = GetResult[0].PhoneNo;
                        txtContactPerson.Text = GetResult[0].ContactPerson;
                        txtAddress.Text = GetResult[0].Address;
                        ddl_status.SelectedValue = GetResult[0].IsActive == true ? "1" : "0";
                        ViewState["ID"] = GetResult[0].VendorID;
                        btnsave.Text = "Update";
                        bindresponsive();
                    }
                }
                if (e.CommandName == "Deletes")
                {
                    ...
                    //    return;
                    //}
                    VendorData objdata = new VendorData();
                    VendorBO objVendorBO = new VendorBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gv_vendor.Rows[i];
                    Label ID = (Label)gr.Cells[0].FindControl("lblID");
                    objdata.VendorID = Convert.ToInt32(ID.Text);
                    ...
                    int Result = objVendorBO.DeleteVendorID(objdata);
                    if (Result == 1)
                    {
                ...
                if (e.CommandName == "activate")
                {
                    ...
                    int Result = objBO.DeleteVendorID(objdata);
                    if (Result == 2)
                    {
        ...
        private void bindgrid(int index)
        {
            ...
            List<VendorData> lstclass = Gv_vendorlist(index, pagesize);
            if (lstclass.Count > 0)
            {
                if (ddl_status.SelectedValue == "0")
                {
                    Gv_vendor.Columns[10].Visible = true;
                }
                else
                {
                    Gv_vendor.Columns[10].Visible = false;
                }
                Gv_vendor.PageSize = pagesize;
               ...
               // lbl_totalrecords.Visible = true;
                ...
                Gv_vendor.VirtualItemCount = lstclass[0].MaximumRows;//total item is required for custom paging
                Gv_vendor.PageIndex = index - 1;
                Gv_vendor.DataSource = lstclass;
                Gv_vendor.DataBind();
                ds = ConvertToDataSet(lstclass);
                TableCell tableCell = Gv_vendor.HeaderRow.Cells[0];
                Image img = new Image();
        ...
        {
            //Responsive 
            Gv_vendor.HeaderRow.Cells[0].Attributes["data-class"] = "expand";
            // GvstudentDetails.HeaderRow.Cells[1].Attributes["data-hide"] = "phone,tablet";
            Gv_vendor.HeaderRow.Cells[2].Attributes["data-hide"] = "phone,tablet";
            Gv_vendor.HeaderRow.Cells[3].Attributes["data-hide"] = "phone,tablet";
            Gv_vendor.HeaderRow.Cells[4].Attributes["data-hide"] = "phone,tablet";
            Gv_vendor.HeaderRow.Cells[5].Attributes["data-hide"] = "phone,tablet";
            //GvClassDetails.HeaderRow.Cells[6].Attributes["data-hide"] = "phone,tablet";
            //  Adds THEAD and TBODY to GridView.
            Gv_vendor.UseAccessibleHeader = true;
            Gv_vendor.HeaderRow.TableSection = TableRowSection.TableHeader;
        ...
        protected void Gv_vendor_Sorting(object sender, GridViewSortEventArgs e)
        {
            try
            {
                ...
                dt = ds.Tables[0];
                {
                    ...
                    Gv_vendor.DataSource = sortedView;
                    Gv_vendor.DataBind();
                    bindresponsive();
                    TableCell tableCell = Gv_vendor.HeaderRow.Cells[ColumnIndex];
                    Image img = new Image();
        ...
        public List<VendorData> Gv_vendorlist(int curIndex, int pagesize)
        {
            VendorData objvendor = new VendorData();
            VendorBO objVendorBO = new VendorBO();
            objvendor.VendorTypeID = Convert.ToInt32(ddl_vendotype.SelectedValue == "" ? "0" : ddl_vendotype.SelectedValue);
            objvendor.PageSize = pagesize;
            objvendor.CurrentIndex = curIndex;
            objvendor.IsActive = ddl_status.SelectedValue == "1" ? true : false;
            return objVendorBO.SearchVendor(objvendor);
        }
        protected void btncancel_Click(object sender, EventArgs e)
        {
            ViewState["ID"] = null;
            ddl_vendotype.SelectedIndex = 0;
            txt_vendor.Text = "";
            txtVendorCode.Text = "";
            txtGSTIN.Text = "";
        ...
        protected DataTable GetDatafromDatabase()
        {
            ...
            List<VendorData> Groupdetail = Gv_vendorlist(1, size);
            List<VendorDatatoXL> grouptoexcel = new List<VendorDatatoXL>();
            int i = 0;
            foreach (VendorData row in Groupdetail)
            {
                VendorDatatoXL EcxeclGroup = new VendorDatatoXL();
                EcxeclGroup.VendorType = Groupdetail[i].VendorType;
                EcxeclGroup.VendorName = Groupdetail[i].VendorName;
