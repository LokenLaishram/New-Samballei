...
namespace Mobimp.Edusoft.Web.EduInvUtility
{
    ...
    public partial class GroupMST : BasePage
    {
        ...
        protected void btnsave_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                GroupBO objBO = new GroupBO();

                obj.Code = txt_code.Text.Trim() == "" ? null : txt_code.Text.Trim();
                obj.Groupname = txt_group.Text.Trim() == "" ? null : txt_group.Text.Trim();
                ...
                if (ViewState["ID"] != null)
                {
                    ...
                    obj.Groupid = Convert.ToInt32(ViewState["ID"].ToString());
                }
        ...
        protected void Gv_group_RowCommand(object sender, GridViewCommandEventArgs e)
        {
            try
            {
                if (e.CommandName == "Edits")
                {
                    ...
                    //    return;
                    //}
                    GroupData objGroup = new GroupData();
                    GroupBO objGroupBO = new GroupBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gv_group.Rows[i];
                    Label ID = (Label)gr.Cells[0].FindControl("lblID");
                    objGroup.Groupid = Convert.ToInt32(ID.Text);
                    List<GroupData> GetResult = objGroupBO.GetGroupbyID(objGroup);
                    if (GetResult.Count > 0)
                    {
                        txt_code.Text = GetResult[0].Code;
                        txt_group.Text = GetResult[0].Groupname;
                ...
                if (e.CommandName == "Deletes")
                {
                    ...
                    int Result = objGroupBO.DeleteGroupbyID(objGroup);
                    if (Result == 1)
                    {
                ...
                {
                    GroupData objgrp = new GroupData();
                    GroupBO objgrpBO = new GroupBO();
                    int i = Convert.ToInt16(e.CommandArgument.ToString());
                    GridViewRow gr = Gv_group.Rows[i];
                    Label ID = (Label)gr.Cells[0].FindControl("lblID");
        ...
        private void bindgrid(int index)
        {
            ...
            List<GroupData> lstclass = getGrouplist(index, pagesize);
            if (lstclass.Count > 0)
            {
                if (ddl_status.SelectedValue == "0")
                {
                    Gv_group.Columns[7].Visible = true;
                }
                else
                {
                    Gv_group.Columns[7].Visible = false;
                }
                Gv_group.PageSize = pagesize;
                ...
                Gv_group.VirtualItemCount = lstclass[0].MaximumRows;//total item is required for custom paging
                Gv_group.PageIndex = index - 1;
                Gv_group.DataSource = lstclass;
                Gv_group.DataBind();
                ds = ConvertToDataSet(lstclass);
                TableCell tableCell = Gv_group.HeaderRow.Cells[0];
                Image img = new Image();
        ...
        {
            //Responsive 
            Gv_group.HeaderRow.Cells[0].Attributes["data-class"] = "expand";
            // GvstudentDetails.HeaderRow.Cells[1].Attributes["data-hide"] = "phone,tablet";
            Gv_group.HeaderRow.Cells[2].Attributes["data-hide"] = "phone,tablet";
            Gv_group.HeaderRow.Cells[3].Attributes["data-hide"] = "phone,tablet";
            Gv_group.HeaderRow.Cells[4].Attributes["data-hide"] = "phone,tablet";
            Gv_group.HeaderRow.Cells[5].Attributes["data-hide"] = "phone,tablet";
            //GvClassDetails.HeaderRow.Cells[6].Attributes["data-hide"] = "phone,tablet";
            //  Adds THEAD and TBODY to GridView.
            Gv_group.UseAccessibleHeader = true;
            Gv_group.HeaderRow.TableSection = TableRowSection.TableHeader;
        ...
        protected void Gv_group_Sorting(object sender, GridViewSortEventArgs e)
        {
            try
            {
                ...
                dt = ds.Tables[0];
                {
                    ...
                    Gv_group.DataSource = sortedView;
                    Gv_group.DataBind();
                    bindresponsive();
                    TableCell tableCell = Gv_group.HeaderRow.Cells[ColumnIndex];
                    Image img = new Image();
        ...
        public List<GroupData> getGrouplist(int curIndex, int pagesize)
        {
            GroupData objGroup = new GroupData();
            GroupBO objGroupBO = new GroupBO();
            objGroup.Code = txt_code.Text == "" ? null : txt_code.Text;
            objGroup.Groupname = txt_group.Text == "" ? null : txt_group.Text;
            objGroup.PageSize = pagesize;
            objGroup.CurrentIndex = curIndex;
            objGroup.IsActive = ddl_status.SelectedValue == "1" ? true : false;
            return objGroupBO.SearchGroup(objGroup);
        }
        ...
        protected void Gv_group_PageIndexChanging(object sender, GridViewPageEventArgs e)
        {
            Gv_group.PageIndex = e.NewPageIndex;
        ...
        protected void ExportoExcel()
        {
            ...
            {
                wb.Worksheets.Add(dt, "GroupList");
                Response.Clear();
        ...
        protected DataTable GetDatafromDatabase()
        {
            ...
            List<GroupData> Groupdetail = getGrouplist(1, size);
            List<GroupDatatoXL> grouptoexcel = new List<GroupDatatoXL>();
            int i = 0;
            foreach (GroupData row in Groupdetail)
            {
                GroupDatatoXL EcxeclGroup = new GroupDatatoXL();
                //EcxeclGroup.GroupID = Groupdetail[i].GroupID;
                EcxeclGroup.Code = Groupdetail[i].Code;
                EcxeclGroup.Groupname = Groupdetail[i].Groupname;
                //EcxeclGroup.AddedBy = Groupdetail[i].Empname;
                //EcxeclGroup.AddedDate = Groupdetail[i].AddedDT.ToString();
                //EcxeclGroup.ModifiedBy = Groupdetail[i].ModEmpName;
                //EcxeclGroup.ModifiedDate = Groupdetail[i].ModifiedDT.ToString();
                //EcxeclGroup.IsActive = Convert.ToString(Groupdetail[i].IsActive == true ? "Yes" : "No");
                //EcxeclGroup.Remarks = Groupdetail[i].Remark;
