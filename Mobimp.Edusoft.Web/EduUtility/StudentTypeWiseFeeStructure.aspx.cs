...
namespace Mobimp.Edusoft.Web.EduUtility
{
    ...
    public partial class StudentTypeWiseFeeStructure : BasePage
    {
        ...
        public List<FeesData> GetFeeDetails(int curIndex, int pagesize)
        {
            ...
            return objFeeBO.SearchBreakupFeesDetails(objfees);
        ...
        protected void GvFeedetails_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            foreach (GridViewRow row in GvFeedetails.Rows)
            {
                try
                {
                    ...
                    TextBox LFine = (TextBox)GvFeedetails.Rows[row.RowIndex].Cells[0].FindControl("txtlatefine");
                    TextBox FineDate = (TextBox)GvFeedetails.Rows[row.RowIndex].Cells[0].FindControl("txtfinedate");
                    if (LFine.Text== "00")
                    {
                        FineDate.Attributes["disabled"] = "disabled";
                    }
                    else
                    {
                        FineDate.Attributes.Remove("disabled");
                    }
                    if (FineDate.Text == "01/01/0001")
                    {
                        FineDate.Text="";
                    }
                    if (FineDate.Text == "01/01/1753")
                    {
                        FineDate.Text = "";
                    }
        ...
        protected void btnupdate_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                foreach (GridViewRow row in GvFeedetails.Rows)
                {
                    ...
                    if((FeeAmount.Text=="00"|| FeeAmount.Text == "0" || FeeAmount.Text == ""))
                    {
                        System.Web.UI.ScriptManager.RegisterClientScriptBlock(this, this.GetType(), "AlertBox", "failalert('" + Messagealert_.Alertmessage("CheckFeeAmount") + "')", true);
                        return;
                    }
        ...
        protected void chkdueallowedstatus_CheckedChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow row in GvFeedetails.Rows)
            {
                try
                {
                    CheckBox ChkDue = (CheckBox)GvFeedetails.Rows[row.RowIndex].Cells[0].FindControl("chkdueallowedstatus");
                    TextBox DueAmt = (TextBox)GvFeedetails.Rows[row.RowIndex].Cells[0].FindControl("txtdueamount");
                    if (ChkDue.Checked == true)
                    {
                        DueAmt.Enabled = true;
                    }
                    else
                    {
                        DueAmt.Enabled = false;
                    }
                }
                catch (Exception ex)
                {
        ...
        protected void txtlatefine_TextChanged(object sender, EventArgs e)
        {
            foreach (GridViewRow row in GvFeedetails.Rows)
            {
                try
