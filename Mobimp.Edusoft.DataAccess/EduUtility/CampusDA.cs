...
namespace Mobimp.Edusoft.DataAccess.EduUtility
{
    public class CampusDA
    {
            ...
            {
                {
                    ...
                    arParms[0] = new SqlParameter("@CampusID", SqlDbType.Int);
                    arParms[0].Value = objclass.CampusID;
                    ...
                    int result_ = SqlHelper.ExecuteNonQuery(GlobalConstant.ConnectionString, CommandType.StoredProcedure, "usp_CMS_Util_UpdateCampusDetailMST", arParms);
            ...
            {
                {
                    ...
                    sqlReader = SqlHelper.ExecuteReader(GlobalConstant.ConnectionString, CommandType.StoredProcedure, "usp_CMS_Util_SeacrhCampusDetailMST", arParms);
                    List<CampusData> lstCampusDetails = ORHelper<CampusData>.FromDataReaderToList(sqlReader);
                    result = lstCampusDetails;
                }
            }
            catch (Exception ex) //Exception of the business layer(itself)//unhandle
            {
            ...
            {
                {
                    ...
                    SqlDataReader sqlReader = null;
                    sqlReader = SqlHelper.ExecuteReader(GlobalConstant.ConnectionString, CommandType.StoredProcedure, "usp_CMS_Util_EditCampusDetailMST", arParms);
                    List<CampusData> lstCampusDetails = ORHelper<CampusData>.FromDataReaderToList(sqlReader);
                    result = lstCampusDetails;
                }
            }
        ...
        public int ActivateCampus(CampusData objclass)
        {
            int result = 0;
            try
            {
                {
                    ...
                    int result_ = SqlHelper.ExecuteNonQuery(GlobalConstant.ConnectionString, CommandType.StoredProcedure, "usp_CMS_Std_ActivateCampusbyID", arParms);
                    if (result_ > 0 || result_ == -1)
            ...
            {
                {
                    ...
                    int result_ = SqlHelper.ExecuteNonQuery(GlobalConstant.ConnectionString, CommandType.StoredProcedure, "usp_CMS_Util_DeleteCampusDetailMST", arParms);
                    if (result_ > 0 || result_ == -1)
