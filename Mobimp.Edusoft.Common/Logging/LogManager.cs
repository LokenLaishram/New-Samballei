...
namespace Mobimp.Edusoft.Common.Logging
{
    public static class LogManager
    {
        ...
        public static void UpdateCmsErrorDetails(Exception ex)
        {
            UpdateCmsErrorDetails(ex, EnumErrorLogSourceTier.DB);
        }

        public static void UpdateCmsErrorDetails(Exception ex, EnumErrorLogSourceTier LogSourceTier)
        {
            if (ex != null)
            {
                if (ex is SqlException)
                {
                    ...
                    LogManager.UpdateCmsErrorDetails(new ErorrLog(LogSourceTier, sqlex.LineNumber, sqlex.Number, (int)sqlex.State, sqlex.Procedure, errorMessages.ToString()));
                }
                else
                {
                    LogManager.UpdateCmsErrorDetails(new ErorrLog(LogSourceTier, 0, 0, 0, "", ex.ToString()));
                }
            }
        }

        public static void UpdateCmsErrorDetails(string ErrorMessageDetails, EnumErrorLogSourceTier LogSourceTier)
        {
            LogManager.UpdateCmsErrorDetails(new ErorrLog(LogSourceTier, 0, 0, 0, "", ErrorMessageDetails));
        }

        public static void UpdateCmsErrorDetails(ErorrLog objErorrLog)
        {
            if (objErorrLog != null)
            {
                SqlConnection con = new SqlConnection(GlobalConstant.ConnectionString);
                con.Open();
                SqlCommand cmd = new SqlCommand("usp_CMS_UpdateErrorLogDetail", con);
